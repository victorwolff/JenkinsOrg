    ...
    withEnv(["HOME=${env.WORKSPACE}"]) {
        ...
        withCredentials([file(credentialsId: SERVER_KEY_CREDENTALS_ID, variable: 'server_key_file')]) {
            ...
            stage('Create Test Scratch Org') {
                rc = command "\"${toolbelt}\" force:org:create --targetdevhubusername victorwolffjobs@gmail.com --setdefaultusername --definitionfile config/project-scratch-def.json --setalias ciorg --wait 10 --durationdays 1"
                if (rc != 0) { 
